...
class Suggester extends React.Component {
  ...
  getSuggestions(value) {
    ...
    if (escapedValue === '') {
      return NewCampaign.activityFields;
    }
  ...
  render() {
    ...
    if (!this.props.value.name && this.props.value.id) {
      for (let i = 0; i < NewCampaign.activityFields.length; i += 1) {
        const elem = NewCampaign.activityFields[i].children.find((child) => {
...
export default observer(Suggester);
